...
#[derive(Debug, Clone, Copy, PartialEq)]
pub enum CompOp { Eq, Neq, Gt, Lt, Gte, Lte }
...
pub fn parse_query(input: &str) -> Result<Query> {
    ...
    let mut pos = 0;
    let mut q = Query {
        select: Vec::new(), where_clause: None, order_by: Vec::new(),
        group_by: Vec::new(), limit: None, labels: Vec::new(),
    };
    while pos < tokens.len() {
        match tokens[pos].to_ascii_uppercase().as_str() {
            "SELECT" => { pos += 1; q.select = parse_select(&tokens, &mut pos)?; }
            "WHERE"  => { pos += 1; q.where_clause = Some(parse_where(&tokens, &mut pos)?); }
            "ORDER"  => {
                expect_next(&tokens, pos + 1, "BY", "ORDER")?;
                pos += 2; q.order_by = parse_order_by(&tokens, &mut pos)?;
            }
            "GROUP" => {
                expect_next(&tokens, pos + 1, "BY", "GROUP")?;
                pos += 2; q.group_by = parse_group_by(&tokens, &mut pos)?;
            }
            "LIMIT" => {
                pos += 1;
                if pos >= tokens.len() { return Err(qerr("expected number after LIMIT")); }
                q.limit = Some(parse_usize(&tokens[pos])?); pos += 1;
            }
            "LABEL" => { pos += 1; q.labels = parse_labels(&tokens, &mut pos)?; }
...
fn tokenize_query(input: &str) -> Result<Vec<String>> {
    ...
    while let Some(&ch) = chars.peek() {
        ...
        if ch == '\'' || ch == '"' {
            ...
            tokens.push(format!("'{s}'")); continue;
        }
        if ch == ',' || ch == '(' || ch == ')' {
            tokens.push(ch.to_string()); chars.next(); continue;
        }
        if matches!(ch, '>' | '<' | '=' | '!') {
            chars.next();
            if let Some(&nx) = chars.peek() {
                let is_two_char = matches!((ch, nx),
                    ('>', '=') | ('<', '=') | ('<', '>') | ('!', '='));
                if is_two_char {
                    tokens.push(format!("{ch}{nx}")); chars.next(); continue;
                }
            }
            tokens.push(ch.to_string()); continue;
        }
        if ch.is_ascii_digit()
            || (ch == '-' && chars.clone().nth(1).is_some_and(|c| c.is_ascii_digit()))
        {
            let mut n = String::new();
            if ch == '-' { n.push('-'); chars.next(); }
        ...
        if ch.is_ascii_alphabetic() || ch == '_' || ch == '*' {
            ...
            while let Some(&c) = chars.peek() {
                if c.is_ascii_alphanumeric() || c == '_' || c == '*' { w.push(c); chars.next(); }
                else { break; }
            }
...
fn parse_select(tokens: &[String], pos: &mut usize) -> Result<Vec<SelectItem>> {
    if *pos < tokens.len() && tokens[*pos] == "*" { *pos += 1; return Ok(vec![]); }
    let mut items = Vec::new();
    loop {
        if *pos >= tokens.len() || is_keyword(&tokens[*pos]) { break; }
        items.push(parse_select_item(tokens, pos)?);
        if *pos < tokens.len() && tokens[*pos] == "," { *pos += 1; } else { break; }
    }
    if items.is_empty() { return Err(qerr("SELECT requires at least one column")); }
    Ok(items)
}

fn parse_select_item(tokens: &[String], pos: &mut usize) -> Result<SelectItem> {
    if *pos >= tokens.len() { return Err(qerr("unexpected end of SELECT")); }
    let upper = tokens[*pos].to_ascii_uppercase();
    if let Some(agg) = parse_agg_name(&upper) {
        let has_parens = *pos + 3 < tokens.len()
            && tokens[*pos + 1] == "(" && tokens[*pos + 3] == ")";
        if has_parens {
    ...
    let col = parse_col_ref(&tokens[*pos])?; *pos += 1;
...
fn parse_where_logic(tokens: &[String], pos: &mut usize, left: WhereExpr) -> Result<WhereExpr> {
    ...
    match tokens[*pos].to_ascii_uppercase().as_str() {
        "AND" => {
            *pos += 1; let r = parse_where_atom(tokens, pos)?;
...
fn parse_where_atom(tokens: &[String], pos: &mut usize) -> Result<WhereExpr> {
    if *pos >= tokens.len() { return Err(qerr("unexpected end of WHERE clause")); }
    let col = parse_col_ref(&tokens[*pos])?; *pos += 1;
    if *pos >= tokens.len() { return Err(qerr("WHERE clause missing operator")); }
    if tokens[*pos].eq_ignore_ascii_case("IS") {
        *pos += 1;
        if *pos >= tokens.len() { return Err(qerr("expected NULL or NOT after IS")); }
        if tokens[*pos].eq_ignore_ascii_case("NOT") {
            ...
            *pos += 1; return Ok(WhereExpr::IsNotNull(col));
        } else if tokens[*pos].eq_ignore_ascii_case("NULL") {
            *pos += 1; return Ok(WhereExpr::IsNull(col));
        }
        return Err(qerr("expected NULL or NOT after IS"));
    }
    let op = parse_comp_op(&tokens[*pos])?; *pos += 1;
    if *pos >= tokens.len() { return Err(qerr("WHERE missing value after operator")); }
    let lit = parse_literal(&tokens[*pos])?; *pos += 1;
...
fn parse_order_by(tokens: &[String], pos: &mut usize) -> Result<Vec<(ColRef, SortOrder)>> {
    ...
    loop {
        ...
        let ord = if *pos < tokens.len() {
            match tokens[*pos].to_ascii_uppercase().as_str() {
                "ASC"  => { *pos += 1; SortOrder::Asc }
                "DESC" => { *pos += 1; SortOrder::Desc }
                _ => SortOrder::Asc,
            }
        } else { SortOrder::Asc };
        specs.push((col, ord));
...
fn parse_group_by(tokens: &[String], pos: &mut usize) -> Result<Vec<ColRef>> {
    ...
    loop {
        if *pos >= tokens.len() || is_keyword(&tokens[*pos]) { break; }
        cols.push(parse_col_ref(&tokens[*pos])?); *pos += 1;
        if *pos < tokens.len() && tokens[*pos] == "," { *pos += 1; } else { break; }
    }
    Ok(cols)
}
...
fn parse_labels(tokens: &[String], pos: &mut usize) -> Result<Vec<(ColRef, String)>> {
    ...
    loop {
        if *pos >= tokens.len() || is_keyword(&tokens[*pos]) { break; }
        let col = parse_col_ref(&tokens[*pos])?; *pos += 1;
        if *pos >= tokens.len() { return Err(qerr("LABEL missing label string")); }
        let lbl = parse_string_literal(&tokens[*pos])?; *pos += 1;
        labels.push((col, lbl));
        if *pos < tokens.len() && tokens[*pos] == "," { *pos += 1; } else { break; }
    }
    Ok(labels)
...
fn parse_col_ref(token: &str) -> Result<ColRef> {
    ...
    let mut idx: usize = 0;
    for ch in upper.chars() { idx = idx * 26 + (ch as usize - 'A' as usize + 1); }
    Ok(idx - 1)
}

fn parse_agg_name(upper: &str) -> Option<AggFunc> {
    match upper {
        "SUM" => Some(AggFunc::Sum), "COUNT" => Some(AggFunc::Count),
        "AVG" => Some(AggFunc::Avg), "MIN" => Some(AggFunc::Min),
        "MAX" => Some(AggFunc::Max), _ => None,
    }
}

fn parse_comp_op(token: &str) -> Result<CompOp> {
    match token {
        "=" => Ok(CompOp::Eq), "!=" | "<>" => Ok(CompOp::Neq),
        ">" => Ok(CompOp::Gt), "<" => Ok(CompOp::Lt),
        ">=" => Ok(CompOp::Gte), "<=" => Ok(CompOp::Lte),
...
fn parse_string_literal(token: &str) -> Result<String> {
    ...
    } else { Err(qerr(&format!("expected quoted string, got '{token}'"))) }
}

fn parse_usize(token: &str) -> Result<usize> {
    token.parse::<usize>().map_err(|_| qerr(&format!("expected integer, got '{token}'")))
}

fn is_keyword(token: &str) -> bool {
    matches!(token.to_ascii_uppercase().as_str(),
        "SELECT" | "WHERE" | "ORDER" | "GROUP" | "LIMIT" | "LABEL")
}

fn expect_next(tokens: &[String], idx: usize, expected: &str, after: &str) -> Result<()> {
    if idx < tokens.len() && tokens[idx].to_ascii_uppercase() == expected { Ok(()) }
    else { Err(qerr(&format!("expected {expected} after {after}"))) }
}
